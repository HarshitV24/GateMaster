...
[Start]
   ↓
[Application Setup (Create Tenant/App)]
   ↓
[User Registration (User joins application)]
   ↓
[User Authentication (Login + Token Generation)]
   ↓
[Role Definition (Admin creates roles)]
   ↓
[Permission Assignment (Permissions mapped to roles)]
   ↓
...
[Access Granted]            [Access Denied]
 ↓                               ↓
[Request Handling]          [403 Forbidden]
 ↓
[End]
```
...
## Data flow
```
Client → API Request → Auth Middleware → Token Validation
      → Fetch User → Fetch Roles → Fetch Permissions
      → Guard Check →  (Yes/No)
...
## Let's keep in touch
- [LinkedIn](https://www.linkedin.com/in/harshit-vaibhav-barla-539857280/)